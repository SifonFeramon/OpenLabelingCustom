...
from yo_object_detection import ObjectDetector


detector = None #ObjectDetector()
probs = []
...
#nikolay#####################################################################################################
def draw_prev_boxes(annotation_paths, coffset):
    try:
        print("getting prew frames")
        global img_objects#, is_bbox_selected, selected_bbox
        img_objects = []
        ann_path = None
        ann_path = next(path for path in annotation_paths if 'YOLO_darknet' in path)
        custom_path = ann_path.replace('test2', 'test1')
        digit = re.findall(r'\d+', custom_path)[-1]
        custom_path = custom_path.replace(digit, str(int(digit) + coffset))
        with open(custom_path) as fp:
...
def overlap(box1, box2):
    try:
        ...
        return (xMin1 < xMax2 or xMax1 > xMin2) and (yMin1 < yMax2 or yMax1 > yMin2) and (zMin1 < zMax2 or zMax1 > zMin2)
    except Exception as e:
        return False

def add_detected(annotation_paths):
    boxes = []
    counter = 10
    while len(boxes) < 1 and counter > 0:
        print('searching objects')
        boxes, scores, class_indices = detector.detect(img)
        counter = counter-1
    for i in range(len(scores)):
        ...
        prob = scores[i]
        global probs
        probs.append([box[0] + box[2]//2, box[1] + box[3]//2, prob])
        print(f"probs len: {len(probs)}")
        print(f"detected: {scores[i]}: {box}")
        color = class_rgb[class_index].tolist()
        flGood = True
        for j in range(len(scores)):
            if j!=i:
                if(overlap(box[i], box[j])):
                    flGood = False
        print(f"Not overlapped: {flGood}")
        if flGood:
            save_bounding_box(annotation_paths, class_index, (xmin, ymin), (xmax, ymax), width, height)
        cv2.rectangle(tmp_img, (xmin, ymin), (xmax, ymax), color, LINE_THICKNESS)

#read prew frame boxes - 3099
#mirror all boxes
#encode and draw boxes
...
def draw_line(img, x, y, height, width, color):
    #b, g, r = color
    #c = choice([0.5, 0.6, 0.7, 0.8, 0.9, 1])
    #ncolor = (int(b*c), int(g*c), int(r*c))
    ...
    if os.path.isfile(ann_path):
        ...
        with open(ann_path) as fp:
            for idx, line in enumerate(fp):
                ...
                if ymax > height:
                    ymax = height
                sub_img = tmp_img[ymin:ymax, xmin:xmax]
                blank = create_blank(abs(xmax) - abs(xmin), abs(ymax) - abs(ymin), color)
                try:
                    res = cv2.addWeighted(sub_img, 0.7, blank, 0.3, 1.0)
                    tmp_img[abs(ymin):abs(ymax), abs(xmin):abs(xmax)] = res
                except Exception as e:
                    print(e)
                    print("shapes: ",sub_img.shape, blank.shape)
                ...
                b, g, r = color
                ncolor = (255 - b, 255 - g, 255 -r)
                cv2.putText(tmp_img, class_name, (xmin, ymin + 15), font, 0.6, ncolor, LINE_THICKNESS, cv2.LINE_AA)
    return tmp_img
...
class LabelTracker():
    ...
    def start_tracker(self, json_file_data, json_file_path, img_path, obj, color, annotation_formats):
        ...
        for frame_path in self.next_frame_path_list:
            path_index = IMAGE_PATH_LIST.index(frame_path)
            next_image = ALLOCATOR.get(path_index)
            # get the new bbox prediction of the object
            success, bbox = tracker.update(next_image.copy())
            if success and bbox[1] < 10:
                lbb = list(bbox)
                lbb[1] = 0
                bbox = tuple(lbb)
                print("bbox stretch to top")
            #nikolay
            hh, ww = tmp_img.shape[:2]
            x, y, w, h = bbox
            ...
            if success:
                ...
                cv2.rectangle(next_image, (xmin, ymin), (xmax, ymax), color, LINE_THICKNESS * 3)
                # save prediction
...
if __name__ == '__main__':    
    ...
    for f in image_load.list_folder_files(INPUT_DIR, os.path.isdir):
        ...
        video_name_ext = os.path.basename(f)
    ...
    cv2.createButton("prepare_yolo", prepare_yolo, None, cv2.QT_CHECKBOX, 1)
    ...
    while True:
        ...
        if dragBBox.anchor_being_dragged is None:
            ...
            # show edges key listener
                ...
                if is_from_video:
                    ...
                    json_file_path = '{}.json'.format(os.path.join(TRACKER_DIR, video_name))
                    file_exists, json_file_data = get_json_file_data(json_file_path)
                    if file_exists and not first_try:
                    ...
                    first_try = False
            # quit key listener
            elif pressed_key in [ord('z'), ord('x'), ord('c')]:
                ...
                if is_from_video:
                    object_list = img_objects[:]
                    draw_prev_boxes(annotation_paths, coffset)
                #img_index = increase_index(img_index, last_img_index)
                #set_img_index(img_index)
                #cv2.setTrackbarPos(TRACKBAR_IMG, WINDOW_NAME, img_index)
        ...
        if point_1[0] != -1:
            ...
            xmin, ymin = point_1
            xmax, ymax, = mouse_x, mouse_y
            if xmin > xmax:
                xmin, xmax = xmax, xmin
            if ymin > ymax:
                ymin, ymax = ymax, ymin
            sub_img = tmp_img[ymin:ymax, xmin:xmax]
            blank = create_blank(xmax - xmin, ymax - ymin, color)
            res = cv2.addWeighted(sub_img, 0.7, blank, 0.3, 1.0)
            if res is not None:
                tmp_img[ymin:ymax, xmin:xmax] = res
            # if second click
            if point_2[0] != -1:
        ...
        for pr in probs:
            x, y, prob = pr
            cv2.putText(tmp_img, "{:.2f}".format(prob[0]), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1)
